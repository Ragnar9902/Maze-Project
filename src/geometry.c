...
void draw_incli_line(t_sdl *sdl, int x, int y, int d, double dir, int hexa_code)
{
    SDL_Color   color;
  ...
  SDL_SetRenderDrawColor(sdl->renderer, color.r, color.g, color.b, SDL_ALPHA_OPAQUE);
  SDL_RenderDrawLine(sdl->renderer, x - (d / 2) * cos(dir * 2 * PI / 360), y + (d / 2) * sin(dir * 2 * PI / 360), x + (d / 2 ) * cos(dir * 2 * PI / 360), y - (d / 2) * sin(dir * 2 * PI / 360));
}
...
void draw_triangle(t_sdl * sdl, int x, int y, size_t h, int w, double dir, int hexa_code)
{
    ...
    {
        draw_incli_line(sdl, x + i * cos((dir * 2 * PI / 360)), y - i * sin((dir * 2 * PI / 360)),w - i * (w/h), dir - 90, hexa_code );
    }
...
void miniMap(t_sdl *sdl, int x, int y, t_raycaster *rc )
{
    ...
    for (size_t i = 0; i < mapWidth ; i++)
    {
        ...
        {
            draw_square(sdl, x + wall_side*i, y - wall_side*j, wall_side, hexa_wall_color(worldMap[i][j]));
        }
        
    }
    int miniCx = (9.1 * rc->player_pos_x) + x + 16;
    int miniCy = (-9.1 * rc->player_pos_y) + y - 4;
    double miniDir = atan(rc->player_dir_y / rc->player_dir_x) * (360/(2 * PI)) + 180;
